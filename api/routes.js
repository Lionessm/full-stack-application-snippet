...
router.post('/api/users', async (req,res) => {
    ...
    if (!parsedBody.password) {
        ...
        res.status(400).json({ error: 'Please provide a value for password'});
        return
    }
    ...
    try {
        const userDetails = {
            ...
            confirmedPassword: parsedBody.password,
        ...
        if (error.name === 'SequelizeValidationError' || error.name === 'SequelizeUniqueConstraintError') {
            ...
            console.log("sequalize validation error or unieque constraint error");
